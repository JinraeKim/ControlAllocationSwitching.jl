function topview(dir_save)
    ...
    savefig(fig, joinpath(dir_save, "topview.png"))
end

function model_description(dir_save)
    multicopter = LeeHexacopterEnv()
    x0 = State(multicopter)()
    length_param = 0.5
    ...
    savefig(fig, joinpath(dir_save, "hexacopter_description.png"))
    display(fig)
end

function prob_description(dir_save)
    multicopter = LeeHexacopterEnv()
    x0 = State(multicopter)()
    ...
    savefig(fig, joinpath(dir_save, "prob_description.png"))
    display(fig)
end

function scheme_description(dir_save)
    l = @layout [a; b]
    t0 = 0.0
    tf = 20.0
    Δt = 0.01
    ...
    savefig(fig, joinpath(dir_save, "scheme_description.png"))
    display(fig)
end
...
function draw_figures()
    ...
    #
    topview(dir_save)
    model_description(dir_save)
    prob_description(dir_save)
    scheme_description(dir_save)
end
